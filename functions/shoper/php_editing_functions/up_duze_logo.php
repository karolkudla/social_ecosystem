...
if ($user['_id'] !== null) { 
	...
	$upload_dir = $user_upload . $uid . $user_shoper_background;
	...
	foreach($_FILES['big_logo']['name'] as $key=>$val) {        
		
	$file_name = $_FILES['big_logo']['name'][$key];
	...
	$upload_image = $upload_dir.$new_name;
		...
		move_uploaded_file($_FILES['big_logo']['tmp_name'][$key],$upload_image);	
		scale_and_compress($upload_dir.$new_name,$upload_dir.'big-'.$new_name,1024,80);
	...
	[ '_id' => new MongoDB\BSON\ObjectID( $uid ) ],
		[ '$set' => [ 
						'shoper.background' => $new_name
		]]
		
	);
	
	echo $user_download.$uid.$user_shoper_background.'big-'.$new_name;
